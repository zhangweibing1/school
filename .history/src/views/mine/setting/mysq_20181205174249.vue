<template>
  <div class="mysq">
    ...
    <van-cell-group>
      <div v-for=" (i)in mysqList" :key="i">
         <div v-for="(list, i) in mysqList" :key="i">
        <van-field
          required
          clearable
          v-model="list[i]['sysSecurityquestion']['content']"
          icon="question"
          placeholder=""
          @click-icon="$toast('密保问题')"
        />
        <van-field v-model="list[i]['answer']" placeholder="请输入答案" /> 
      </div>
    </van-cell-group>
      <div class="btn-group">
