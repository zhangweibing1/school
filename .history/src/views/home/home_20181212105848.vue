...
export default {
  ...
  methods: {
    async getBaNoticeAndNew() {
      ...
      if (data.notice != null) {
          const content = data.notice.records[0].content;
          content.replace(/<\/?.+?>/g, '').replace(/ /g, '').substring(0, 20);
        data.notice.records[0].content = `${content} ...`
           + '...';
      ...
      if (data.news != null) {
        data.news.records[0].content = data.news.records[0].content
