<template>
  <div class="noticeList">
    ...
    <van-tabs v-model="active">
      <van-tab title="通知公告" @click="choseTab(2)">
        <div class="wrapper">
          <span v-for="n in noticeList" :key="n">
            <div class="one">
            <h4>
              {{n.title}}
              <span class="bd">未读</span>
            </h4>
            <p class="neirong">青峰教务系统是青峰软件有限公司专为小学、初中和高中研发的智能化校园教务办公系统，旨在解决学校教务办公、家长查询...</p>
            <div class="date">
              <p class="time">
          ...
          <div class="one">
            <h4>
              公司纪律声明公司纪律声明公司纪律声明公司纪律声明公司纪律声明
              <span class="yd">已读</span>
            </h4>
            <div class="date">
              <p class="time">
                <img :src="require('@/assets/time.png')" alt> 5分钟前
              </p>
            </div>
          </div>
          <div class="one">
      ...
      <van-tab title="新闻资讯" @click="choseTab(3)">
  ...
  methods: {
    ...
    async queryData(param) {
      ...
      if (param.type === "2") {
        ...
        this.noticeList = record;
      } else {
        let record = data.news.record;
        for( const i in record){
          records[i].content = records[i].content
          .replace(/<\/?.+?>/g, '').replace(/ /g, '').substring(0, 20) + '...';
        records[i].sendTime = records[i].sendTime.substring(0, 10);
        }
        this.newsList = record;
      }
    },
  },
};
