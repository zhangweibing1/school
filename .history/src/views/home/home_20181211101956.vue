...
export default {
  ...
  methods: {
    async getBaNoticeAndNew() {
      ...
      if (data.notice != null) {
        let b = '';
        data.notice.records[0].content = b.concat(data.notice.records[0].content
      ...
      let a = '';
      if (data.news != null) {
        data.news.records[0].content = a.concat(data.news.records[0].content
          .replace(/<\/?.+?>/g, '').replace(/ /g, '').substring(0, 20), '...');
    ...
    detail(value) {
      if (value === '1') {
        this.$router.push(concat('/newsDetail/' , this.noticeObject.id));
      } else {
        this.$router.push(concat('/newsDetail/' + this.newObject.id));
      }
    },
    moreThing(type) {
      this.$router.push(concat('/noticeList/' + type));
    },
  },
};
</script>
