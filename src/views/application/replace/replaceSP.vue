<template>
  <div class="replaceSP">
    ...
    <div class="btn-group">
      <van-button size="large" class="btn blue" @click="jump('agreefdy')">同意</van-button>
      <van-button size="large" class="btn" @click="jump('disagreefdy')">拒绝</van-button>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    if (!this.$route.params.item) {
      this.navback = true;
      this.myId.id = this.appJSObject.getDetailId();
    } else {
